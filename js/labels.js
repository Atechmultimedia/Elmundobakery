...
function openLabelDesigner(productId) {
  ...
  labelState = {
    ...
    // Old saved choices map onto the richer artwork that replaced them
    art: ({ loaf: "golden", boule: "crusty" }[s.label_art] || s.label_art || "golden"),
...
function labelHtml(product, size, isPreview) {
  ...
  if (tpl.wantsArt && labelState.art && labelState.art !== "none" && LABEL_ART[labelState.art]) {
    ...
    // Photos are cropped to a circle, so they get a square box; drawings are wide.
    const artDef = LABEL_ART[labelState.art];
    const artH = Math.max(4, size.h * (round ? 0.22 : 0.26) * (tpl.wantsLogo ? 0.8 : 1));
    const artW = artDef.photo ? artH : artH * 1.6;
    lines.push(`<div class="lb-art" style="height:${artH}mm;width:${artW}mm;">
      ${artDef.svg(labelState.colors, labelState.colors.bg)}
    </div>`);
  }
